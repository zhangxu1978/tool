        ...
        function calculateResistances(initialRoots, finalRoots, variations) {
            ...
            Object.entries(finalRoots).forEach(([element, state]) => {
                if (state === '废') {
                    // 增加废灵根的减益效果，如果大于0才减30，让废属性的负面影响更加明显
                    if(resistances[element]>0){
                        resistances[element] -= 30;
                        console.log(`${element}废灵根抗性-15`);
                    }else{
                         resistances[element] -= 15;
                        console.log(`${element}废灵根抗性-15`);
                    }
        ...
        function calculateElementBonus(initialRoots, finalRoots, variations) {
            ...
            Object.entries(finalRoots).forEach(([element, state]) => {
                if (state === '废') {
                    elementBonus[element] -= 15;
                    console.log(`${element}废灵根减少元素加成15`);
                }
            });
            ...
            if (variations && variations.length > 0) {
                variations.forEach(variation => {
                    ...
                    if (elementMatch && elementMatch.length >= 5) {
                        ...
                        // 为两个元素增加15%的加成
                        elementBonus[element1] += 15;
                        console.log(`${element1}变异加成：(15)`);
                        elementBonus[element2] += 15;
                        console.log(`${element2}变异加成 (15)`);
                    }
                });
            }
        ...
        function processRelationships(roots) {
            ...
            // 处理异化灵根（1/10的几率）如果有废灵根，无法变异
            const mutatedRoots = [];
            if (mutationInfo.length > 0&&!elementsToDisable.length) {
